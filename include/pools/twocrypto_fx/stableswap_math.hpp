...
// Mirrors contracts/twocrypto-ng/contracts/main/StableswapMath.vy (immutable reference).
#pragma once
...
// Primary template: floating-point (A_MULTIPLIER and PRECISION are exactly
// representable in every fp type). uint256 has its own specialization with the
// 1e18 fixed-point PRECISION.
template <typename T>
struct MathTraits {
    static_assert(std::is_floating_point_v<T>, "MathTraits primary template is floating-point only");
...
struct MathOpsCommon {
    ...
    static T get_y_unchecked(
        ...
        if constexpr (std::is_floating_point_v<T>) {
            // The Newton loop below is Newton's method on y^2 + (b-D)y - c = 0;
            // take its positive root directly. Conjugate form for q < 0 avoids
            // cancellation when |q| dominates c.
            const T q = D - b;
            const T s = std::sqrt(q * q + T(4) * c);
            const T y = (q >= T(0)) ? (q + s) / T(2) : (T(2) * c) / (s - q);
            return y;
        }
    ...
    static T newton_D(
        ...
        // vy newton_D entry guard: positive balances within 10_000:1.
        {
            const T hi = _xp[0] > _xp[1] ? _xp[0] : _xp[1];
            const T lo = _xp[0] > _xp[1] ? _xp[1] : _xp[0];
            if (!(lo > T(0)) || !(hi / lo < T(10000))) {
        ...
        T D   = S;
        if constexpr (std::is_floating_point_v<T>) {
            // Warm start: callers pass the pre-op D, which is within bps of
            // the new root. The iteration is Newton on a convex function, so
            // any positive seed converges; a near-root seed does so in 1-2
            // steps. uint256 keeps the cold S seed for Vyper parity.
            if (K0_prev > T(0)) {
                D = K0_prev;
            }
        } else {
        ...
        // vy raises after 255 iterations; float Newton with a relative
        // tolerance can stall at ulp level, so floats keep the last iterate.
