...
class CppPoolRunner:
    """Run ``benchmark_harness_i`` without touching source or build files."""
    ...
    def harness_path(self) -> Path:
        path = self._harness_override or (self.build_dir / "benchmark_harness_i")
        if not path.is_file():
            raise FileNotFoundError(
                f"exact parity harness is missing: {path}; build the optional parity target "
...
def run_cpp_pool(
    ...
    *,
    ...
    result = runner.run_benchmark(pool_configs_file, sequences_file, output_file)
