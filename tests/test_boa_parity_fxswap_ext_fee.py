"""Exact uint256 C++/Boa parity authority for all policy modes."""
...
from twocrypto_parity.generate_data import generate_action_sequences
...
def _require_cpp_harness() -> Path:
    """Require a separately built harness; never configure or build it here."""
...
def _pool(name: str, policy_kind: str) -> dict:
    return {
        "name": name, "A": "400000", "gamma": "145000000000000", "mid_fee": "26000000",
        "out_fee": "45000000", "fee_gamma": "230000000000000",
        "adjustment_step_min": "1000000000000", "adjustment_step_max": "100000000000000000",
        "ma_time": "866", "reserved_profit_fraction": "5000000000", "admin_fee": "5000000000",
        "initial_price": "1000000000000000000", "policy": {"kind": policy_kind},
...
def _sequence(pool_configs: list[dict]) -> dict:
    random.seed(7)
    sequences = generate_action_sequences(64, 1_700_000_000, pool_configs)
    sequences[0]["name"] = "fxswap_ext_fee_generated_core_actions"
    return {"seed": 7, "sequences": sequences}
...
def _run_cpp(pools_path: Path, sequences_path: Path, output_path: Path) -> dict:
    ...
    subprocess.run(
        [str(harness), str(pools_path), str(sequences_path), str(output_path)],
        cwd=core_root(), check=True, capture_output=True, text=True,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"policy": {"kind": "fixed_fee", "fee_bps": []}}, "policy fee_bps must be a string or number"),
    ],
)
def test_cpp_harness_rejects_stale_or_malformed_policy_config(tmp_path: Path, pool_patch: dict, expected_error: str) -> None:
    harness = _require_cpp_harness()
    pool = _pool("invalid_config", "none")
    pool.update(pool_patch)
    pools_path, sequences_path, cpp_output = (tmp_path / name for name in ("pools.json", "sequences.json", "cpp.json"))
    _write_json(pools_path, {"pools": [pool]})
    _write_json(sequences_path, _sequence([pool]))
    proc = subprocess.run(
        [str(harness), str(pools_path), str(sequences_path), str(cpp_output)],
        cwd=core_root(), check=False, capture_output=True, text=True,
    )
    assert proc.returncode == 0
    result = json.loads(cpp_output.read_text(encoding="utf-8"))["results"][0]["result"]
...
def _action_at(actions: list[dict], action_idx: int) -> dict:
    return actions[action_idx] if action_idx < len(actions) else {"error": "no action for state index", "n_actions": len(actions)}


def _assert_state_equal(pool_name: str, action_idx: int, action: dict, cpp: dict, boa: dict, prev_cpp: dict | None, prev_boa: dict | None) -> None:
    mismatches = {field: {"cpp": cpp.get(field), "boa": boa.get(field)} for field in STATE_FIELDS if cpp.get(field) != boa.get(field)}
    mismatch_fields = set(mismatches)
    assert not mismatches, {
        "pool": pool_name, "action_idx": action_idx, "action": action,
        "previous_matching_fields": {"cpp": _field_subset(prev_cpp, mismatch_fields), "boa": _field_subset(prev_boa, mismatch_fields)},
...
def test_boa_cpp_uint256_parity_for_fxswap_ext_fee_policy_modes(tmp_path: Path) -> None:
    ...
    pool_configs = [_pool("policy_none", "none"), _pool("policy_twocrypto", "twocrypto_policy"), _pool("policy_zero_stub", "zero_stub"), _pool("policy_oracle_x2", "oracle_x2")]
    sequence = _sequence(pool_configs)
    _assert_sequence_mix(sequence)
    pools_path, sequences_path, cpp_output, boa_output = (tmp_path / name for name in ("pools.json", "sequences.json", "cpp.json", "boa.json"))
    ...
    for pool_name in sorted(cpp_states):
        ...
        for action_idx, (cpp, boa) in enumerate(zip(cpp_states[pool_name], boa_states[pool_name])):
            _assert_state_equal(pool_name, action_idx, _action_at(actions, action_idx), cpp, boa, cpp_states[pool_name][action_idx - 1] if action_idx > 0 else None, boa_states[pool_name][action_idx - 1] if action_idx > 0 else None)
